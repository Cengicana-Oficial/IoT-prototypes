...
// ===== DEEP SLEEP =====
#define INTERVALO_US 900000000ULL // 15 minuto en microsegundos
...
float leer_pluviometro() {
  ...
  while (bytesRead < 7 && millis() - t < 1000) {
    if (rs485.available()) response[bytesRead++] = rs485.read();
  }
  ...
  uint16_t crc_resp = modbus_crc(response, 5);
  if ((crc_resp & 0xFF) != response[5] || ((crc_resp >> 8) & 0xFF) != response[6]) {
...
// ============================================================
// SETUP - corre cada vez que despierta
// ============================================================
void setup() {
  Serial.begin(115200);
  delay(500);
  ...
  // Transmitir
  Serial.printf("Enviando (FCnt=%lu, diff=%.1f mm)... ", fCntUp, mm_diff);
  ...
  Serial.printf("Durmiendo 1 minuto...\n\n");
  esp_deep_sleep(INTERVALO_US);
}

void loop() {
  // Con deep sleep el loop nunca corre
}